...
describe('toxicity-engine', () => {
  ...
  test('large trade triggers hard cancel', () => {
    expect(checkHardToxicityCancel(
      ...
      { cancelIfMidpointMoves10sCentsGte: 1.5, cancelIfMidpointMoves60sCentsGte: 3.0, cancelIfLargeTradeUsdGte: 1000, cancelIfHashChanges10sGte: 8, cancelIfSpreadTicksLte: 1, cancelIfBookStaleMsGte: 2000, cooldownAfterCancelSeconds: 20 }
    )).toBe(true);
  });
